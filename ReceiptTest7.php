...
class ReceiptTest extends TestCase {
	...
	public function testPostTaxTotal() { // Maksu korrektse lisamisse kontroll-funktsioon;
		$items = [1,2,5,8]; // Muutujatele
		$tax = 0.20;			// antakse
		$coupon = null;				// väärtused; 
		$Receipt = $this->getMockBuilder('TDD\Receipt')  // Receipt class alusel luuakse Mock-objekt;
			->setMethods(['tax', 'total'])
			->getMock(); 
		$Receipt->expects($this->once()) // Tax meetod kutsutakse üks kord välja;
			->method('total') // Mock objekti meetod "total";
			->with($items, $coupon) // Argumendid;
			->will($this->returnValue(10.00)); // Antud väljund;
		$Receipt->expects($this->once()) 
			->method('tax')
			->with(10.00, $tax)
			->will($this->returnValue(1.00));
		$result = $Receipt->postTaxTotal([1,2,5,8], 0.20, null); // Mock meetod argumentidega;
